 ...
 */
 ...
 /**
 * Class WPCF7R_html
 * A Mainly static functions class to create html fregments
 *
 * @version  1.0.0
 */
class WPCF7R_html{
     ...
     * @param string $mail_tags [description]
     *
     * @version  1.0.0
     */
    ...
    /**
     * [get_conditional_row_group_end description]
     * @return [type] [description]
     *
     * @version  1.0.0
     */
     ...
     * @param  [type]  $group_block_key  [description]
     * @param  [type]  $group_block      [description]
     * @param  [type]  $active_tab_title [description]
    ...
    public static function get_conditional_row_template( $block_key = '' , $group_key = '' , $group_row = '' , $row_fields = array() , $prefix ){
        ...
        <tr class="row-template">
            <td>
                <select class="wpcf7r-fields" name="wpcf7-redirect<?php echo $prefix;?>[blocks][<?php echo $block_key;?>][groups][<?php echo $group_key;?>][<?php echo $group_row;?>][if]">
            ...
            <td>
                <select class="compare-options" name="wpcf7-redirect<?php echo $prefix;?>[blocks][<?php echo $block_key;?>][groups][<?php echo $group_key;?>][<?php echo $group_row;?>][condition]">
            ...
            <td>
                ...
                foreach( WPCF7R_Form::get_mail_tags() as $mail_tag ):?>
                    <?php if( $mail_tag->raw_values ):?>
                        ...
                        <select class="group_row_value group_row_value_select" style="<?php echo $row_fields['if'] != $mail_tag['name'] ? 'display:none;' : '';?>" data-rel="<?php echo $mail_tag['name'];?>">
                            <option value=""><?php _e('Select' , 'wpcf7-redirect' );?></option>
                            <?php foreach( $mail_tag->raw_values as $value ):?>
                                <?php $value = explode('|' , $value );?>
                                <option value="<?php echo $value[0];?>" <?php if( isset( $row_fields['value'] ) ) : selected( $row_fields['value'] , $value[0] ); endif;?>>
                                    <?php echo isset( $value[1] ) && $value[1] ? $value[1] : $value[0];?>
                                </option>
                            <?php endforeach;?>
                        </select>
                    <?php endif;?>
                ...
                <input type="text" class="group_row_value wpcf7-redirect-value"
                       name="wpcf7-redirect<?php echo $prefix;?>[blocks][<?php echo $block_key;?>][groups][<?php echo $group_key;?>][<?php echo $group_row;?>][value]"
                       value="<?php echo isset( $row_fields['value'] ) ? esc_html($row_fields['value']) : '';?>"
                       placeholder="<?php _e('Your value here');?>" style="<?php echo $select_visible ? 'display:none;' : '';?>">
            </td>
            <td>
    ...
    public static function render_field( $field , $prefix ){
        switch($field['type']){
            ...
            case 'select':
                ...
                break;
            case 'tags_map':
                $template = WPCF7_PRO_REDIRECT_FIELDS_PATH . 'field-tags_map.php';
                break;
            case 'debug_log':
                $template = WPCF7_PRO_REDIRECT_FIELDS_PATH . 'field-debug_log.php';
                break;
            case 'json':
