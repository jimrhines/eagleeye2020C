...
if( !class_exists( 'vxcf_netsuite_pages' ) ) {
...
* @since       1.0.0
*/
class vxcf_netsuite_pages   extends vxcf_netsuite{
    public $ajax=false;
...
global $pagenow; 
 ...
 add_filter("admin_menu", array($this, 'create_menu'), 51);
...
public function add_bulk_send_crm_callcenter($list){ 
 $list['send_netsuite']=__('Send to Netsuite','contact-form-netsuite-crm');
  return $list;  
}
public function bulk_send_crm_callcenter($msg,$action,$ids,$type){ 
    if($action == 'send_netsuite'){
...
$entry['__vx_id']=$entry['id'];
        ...
        if(empty($notice)){
            $notice='Nothing Sent to Netsuite';
        }
...
$push=$this->push($entry_detail,$form,'add_note');
  ...
  ?>
  <div class="vx_row">
  <label><input type="checkbox" name="<?php echo $this->id ?>_send" value="yes"> <?php _e('Send to Netsuite','contact-form-netsuite-crm') ?></label>
  </div>
  <?php  
}
    /**
...
$lead['__vx_id']=$id;
   ...
   $slug=$this->get_slug();
      if ( $file == $slug ) {
          $settings_link=$this->link_to_settings();
            array_unshift( $links, '<a href="' .$settings_link. '">' . __('Settings', 'contact-form-netsuite-crm') . '</a>' );
        }
        return $links;
   }
  /**
  ...
  */
  ...
  // Adding submenu if user has access
        $page_title =__('Netsuite','contact-form-netsuite-crm');
        $capability = $this->id."_read_feeds"; 
       add_submenu_page('vxcf_leads',$page_title,$page_title,$capability,$this->id,array( $this,'mapping_page'));
...
$class='';
 if($res){
       $msg=__('Account Deleted Successfully','contact-form-netsuite-crm');
  $class='updated';   
 }else{
       $msg=__('Error While Removing Account','contact-form-netsuite-crm');
  $class='error';      
 }
...
die();
  ...
  $msg=__('You do not have permissions to export logs','contact-form-netsuite-crm');
  ...
  $fields=array(); $field_titles=array("#",__('Status','contact-form-netsuite-crm'),__('Netsuite ID','contact-form-netsuite-crm') ,__('Entry ID','contact-form-netsuite-crm'),__('Description','contact-form-netsuite-crm'),__('Time','contact-form-netsuite-crm'));
  ...
  check_admin_referer('vx_nonce','vx_nonce');
  if(!current_user_can($this->id."_edit_settings")){ 
  $msg=__('You do not have permissions to clear logs','contact-form-netsuite-crm');
  $this->display_msg('admin',$msg);
  return;   
  }
  $data=$this->get_data_object();
  $clear=$data->clear_logs();
       ...
       $msg=__('Error While Clearing Netsuite Logs','contact-form-netsuite-crm');
      $level="error";
      if(!empty($clear)){
      $msg=__('Netsuite Logs Cleared Successfully','contact-form-netsuite-crm');   
      $level="updated";
      }
  ...
  //
  self::$tooltips = array(
 'vx_feed_name' =>  __('Enter feed name of your choice.', 'contact-form-netsuite-crm'),
  'vx_sel_object' => __('Select the Object to Create when a Form is Submitted.', 'contact-form-netsuite-crm'),
   'vx_sel_account' =>__('Select the Netsuite account you would like to export entries to.', 'contact-form-netsuite-crm'),
  'vx_sel_form' => __('Select the Contact Form you would like to integrate with Netsuite. Contacts generated by this form will be automatically added to your Netsuite account.', 'contact-form-netsuite-crm'),
  
  'vx_map_fields' => __('Associate your Netsuite fields to the appropriate Contact Form fields.', 'contact-form-netsuite-crm'),
  
  'vx_optin_condition' =>__('When the opt-in condition is enabled, form submissions will only be exported to Netsuite when the condition is met. When disabled all form submissions will be exported.', 'contact-form-netsuite-crm'),
  
  'vx_manual_export' => __('If you do not want all entries sent to Netsuite, but only specific, approved entries, check this box. To manually send an entry to Netsuite, go to Entries, choose the entry you would like to send to Netsuite, and then click the "Send to Netsuite" button.', 'contact-form-netsuite-crm'),
  
    'vx_entry_notes' => __('Enable this option if you want to synchronize Contact Form entry notes to Netsuite Object notes. For example , when you add a note to a Contact Form entry, it will be added to the Netsuite Object selected in the feed.', 'contact-form-netsuite-crm'),
    
      'vx_primary_key' => __('Which field should be used to update existing objects?', 'contact-form-netsuite-crm'),
      
  
  'vx_disable_logs'=>__('When an order is sent to Netsuite we store that order information in the database and show it in the Netsuite Log. Check this box if you do not want to save the exported order information in the logs.','contact-form-netsuite-crm'),

  'vx_sel_folder'=>__('Select a folder where you want to add uploaded files.','contact-form-netsuite-crm'),
  'vx_sel_status'=>__('Enable this option if you want to assign a stage to customer.','contact-form-netsuite-crm'),
  'vx_status_check'=>__('Enable this option if you want to assign a status to customer.','contact-form-netsuite-crm'),
  'vx_status_list'=>__('Get Lead Status list from netsuite.','contact-form-netsuite-crm'),
  'vx_folders_list'=>__('Get Folders list from netsuite.','contact-form-netsuite-crm'),
  'vx_assign_person'=>__('Enable this option if you want to assign a person to this object.','contact-form-netsuite-crm'),
   
       'vx_priority'=>__('Which Priority should be assigned to this object.','contact-form-netsuite-crm'),
  'vx_status'=>__('Which Status should be assigned to this object.','contact-form-netsuite-crm'),
   'vx_assign_company'=>__('Enable this option if you want to assign an company to this object.','contact-form-netsuite-crm'),
   'vx_sel_company'=>__('Select Company feed. Company created by this feed will be assigned to this object','contact-form-netsuite-crm'),
   
   
   'vx_owner_check'=>__('Enable this option if you want to assign another object owner.','contact-form-netsuite-crm'),
   'vx_owners'=>__('Get Users list from Netsuite','contact-form-netsuite-crm'),
   'vx_order_notes'=>__('Enable this option if you want to synchronize WooCommerce Order notes to Netsuite Object notes. For example, when you add a note to a WooCommerce Order, it will be added to the Netsuite Object selected in the feed.','contact-form-netsuite-crm'),
   'vx_sel_owner'=>__('Select a user as a owner of this object','contact-form-netsuite-crm'),
   
       'vx_entry_note'=>__('Check this option if you want to send more data as CRM entry note.', 'contact-form-netsuite-crm'),
   'vx_note_fields'=>__('Select fields which you want to send as a note', 'contact-form-netsuite-crm'),
   'vx_disable_note'=>__('Enable this option if you want to add note only for new CRM entry', 'contact-form-netsuite-crm')
...
<div class="table_head_i"><?php _e('Capsule', 'contact-form-netsuite-crm'); ?></div>  
<div class="vx_group">
...
echo $this->format_log_msg($log);
 ...
 $msg=__('You do not have permissions for this action','contact-form-netsuite-crm');
...
if( $this->has_feed($form_id)) { 
 $boxes['netsuite_crm']=array('title'=>'<i class="fa fa-plug"></i> '.__('Netsuite', 'contact-form-netsuite-crm'),'callback'=>array($this,'meta_box_html'));  
} return $boxes; 
}
...
public function has_feed($form_id) {
 ...
 if(!empty($info['meta']) ){
  ...
  switch($action){
    ...
    $data['crm_sel_folder']=$this->gen_select($folders,$folder_sel,__('Select Folder','contact-form-netsuite-crm'));
  ...
  $res['data']=$data;   
   ...
   case"refresh_status":
    ...
    $data['crm_sel_user']=$this->gen_select($users,$status_sel,__('Select Status','contact-form-netsuite-crm'));
  ...
  $res['data']=$data;   
    ...
    $data['crm_note_type']=$this->gen_select($users,$status_sel,__('Select Type','contact-form-netsuite-crm'));
  ...
  $res['data']=$data;   
    ...
    $data['crm_sel_emp']=$this->gen_select($users,$status_sel,__('Select Person','contact-form-netsuite-crm'));
  $meta['emp_list']=$users;  
  $res['data']=$data;   
      break;
...
if(isset($res['error'])){
    $res['status']='error';
    if(empty($res['error'])){
    $res['error']=__('Unknown Error','contact-form-netsuite-crm');
    }
}
  die(json_encode($res));    
  }
  ...
  */
  public  function mapping_page(){
      ...
      wp_enqueue_style('vxc-tooltip');
         $tabs=array('feeds'=>__('Netsuite Feeds','contact-form-netsuite-crm') , 'logs'=>__('Netsuite Log','contact-form-netsuite-crm') , 'accounts'=>__('Netsuite Accounts','contact-form-netsuite-crm'), 'settings'=>__('Settings','contact-form-netsuite-crm'));
  ...
  $(this).text('<?php _e('Show Key','contact-form-netsuite-crm') ?>');  
  $(this).removeClass('vx_hidden');
  key.attr('type','password');  
  }else{
  $(this).text('<?php _e('Hide Key','contact-form-netsuite-crm') ?>');  
...
start_tooltip();  
  ...
  */
  ...
  _e('You do not have permissions to access this page','contact-form-netsuite-crm');    
  return;
  }
 ...
 $id = absint($this->post("action_argument"));
  ...
  <p>
  <?php esc_html_e("Feed deleted.", 'contact-form-netsuite-crm') ?>
  </p>
  </div>
  <?php
  }
  ...
  <div class="updated fade" style="margin:10px 0;">
  <p>
  <?php esc_html_e("Feeds deleted.", 'contact-form-netsuite-crm') ?>
  </p>
  </div>
  <?php
  }
...
$count=$this->data->delete_log($log_ids);
  $this->screen_msg(sprintf(__('Successfully Deleted %d Item(s)','contact-form-netsuite-crm'),$count));  
    }
  ...
  if(!empty($entry_id) && class_exists('vxcf_form')){
       $entry=$this->get_cf_entry_detail($entry_id);
  }else{
      //
  ...
  if(!empty($entry) && is_array($entry)){ 
    ...
    $push=array('class'=>'error','msg'=>__('Entry Not Found','contact-form-netsuite-crm'));  
  }
  ...
  case"time": $time_order=$order_icon; $time_class="";   break;    
    ...
    $bulk_actions=array(""=>__('Bulk Action','contact-form-netsuite-crm'),"delete"=>__('Delete','contact-form-netsuite-crm'),
  'send_to_crm_bulk'=>__('Send to Netsuite','contact-form-netsuite-crm'),'send_to_crm_bulk_force'=>__('Force Send to Netsuite - Ignore filters','contact-form-netsuite-crm'));
  $base_url=$this->get_base_url();
   $objects=$this->get_objects(); 
      $statuses=array("1"=>__("Created",'contact-form-netsuite-crm'),"2"=>__("Updated",'contact-form-netsuite-crm'),"error"=>__("Failed",'contact-form-netsuite-crm'),"4"=>__("Filtered",'contact-form-netsuite-crm'),"5"=>__("Deleted",'contact-form-netsuite-crm')); 
...
foreach($map as $field_k=>$field_v){
  ...
  $sel_fields=array(""=>__("Standard Field",'contact-form-netsuite-crm'),"value"=>__("Custom Value",'contact-form-netsuite-crm'));
  ...
  */
  ...
  public function set_logging_supported($plugins) {
      $slug=$this->plugin_dir_name(); 
        $plugins[$slug] = __('Netsuite','contact-form-netsuite-crm');
        return $plugins;
    }
  /**
...
$this->ajax=true;
  ...
  $msg=__("Please Choose Object",'contact-form-netsuite-crm');
  }else  if(empty($_REQUEST['form_id'])){
  $msg=__("Please Choose Form",'contact-form-netsuite-crm');
  }
  if($msg !=""){
  ?>
...
/*  $object=$this->post('object',$feed);
  ...
  $msg=__("Please Choose Account",'contact-form-netsuite-crm');
  }else  if(empty($_REQUEST['form_id'])){
  $msg=__("Please Choose Form",'contact-form-netsuite-crm');
  }
  if($msg !=""){
...
public function get_objects_ajax(){
  ...
  $modules=$this->get_objects($api_type, $crm,true); 

$field_options="<option>".__("Select Object",'contact-form-netsuite-crm')."</option>"; 
  if(is_array($objects)){
...
die();
  ...
  */
  ...
  if(!current_user_can($this->id.'_read_settings')){
  $msg_text=__('You do not have permissions to access this page','contact-form-netsuite-crm');   
  ...
  $this->uninstall();
  $uninstall_msg=sprintf(__("Contact Form Netsuite Plugin has been successfully uninstalled. It can be re-activated from the %s plugins page %s.", 'contact-form-netsuite-crm'),"<a href='plugins.php'>","</a>");
$this->screen_msg($uninstall_msg);
  ...
  $meta=isset($_POST['meta']) ? $this->post('meta') : array();
   ...
   $msgs['submit']=array('class'=>'updated','msg'=>__('Settings Changed Successfully','contact-form-netsuite-crm'));
...
include_once(self::$path . "templates/settings.php");
  ...
  */
  ...
  return;
   ...
   $msg= 'The SOAP PHP extension is not loaded. Please modify the extension settings in php.ini accordingly.';
  $this->display_msg('admin',$msg);
  return;
     }
  $is_section=apply_filters('add_page_html_'.$this->id,false);
 ...
 $offset=$this->time_offset();
  ...
  check_admin_referer("vx_nonce");
  if(!current_user_can($this->id."_edit_settings")){ 
  _e('You do not have permissions to save settings','contact-form-netsuite-crm');
  return;   
  }
  $msgs['submit']=array('class'=>'updated','msg'=>__('Settings Changed Successfully','contact-form-netsuite-crm'));
  $valid_email=true;
  ...
  if(!$valid_email){
      $msgs['submit']=array("class"=>"error","msg"=>__('Invalid Email(s)','contact-form-netsuite-crm'));
  }
...
$info=$this->validate_api($info,$force_check);
  ...
  if($con_class != "updated" ){
      $msg=__('Connection to Netsuite is NOT Working','contact-form-netsuite-crm');  
  }else{
     $msg=__('Connection to Netsuite is Working','contact-form-netsuite-crm');   
  }
  $title=__('Test Connection: ','contact-form-netsuite-crm');
...
$feed= $this->data->get_feed($fid); 
  ...
  check_admin_referer("vx_nonce");
  if(!current_user_can($this->id.'_edit_feeds')){
  _e('You do not have permissions to edit/save feed','contact-form-netsuite-crm'); 
  return;
  }
  //
...
if($is_valid){
      ...
      $msgs['save']=array('class'=>'updated','msg'=>sprintf(__("Feed Updated. %sback to list%s", 'contact-form-netsuite-crm'), '<a href="'.$feed_link.'">', "</a>"));
}
else{
  $msgs['save']=array('class'=>'error','msg'=>__("Feed could not be updated. Please enter all required information below.", 'contact-form-netsuite-crm'));
...
if(is_array($tags)){
  ...
  $fields['__vx_id']=array('id'=>'__vx_id','label'=>__('Entry ID','contact-form-netsuite-crm')); 
$fields['_vx_form_id']=array('id'=>'_vx_form_id','label'=>__('Form ID','contact-form-netsuite-crm')); 
$fields['_vx_form_name']=array('id'=>'_vx_form_title','label'=>__('Form Title','contact-form-netsuite-crm')); 

  $this->fields=$fields=array('cf'=>array("title"=>__('Contact Form Fields','contact-form-netsuite-crm'),"fields"=>$fields));
...
if(is_array($fields_arr['fields'])){
  ...
  */
  ...
  $modules=array(""=>__("Select Object",'contact-form-netsuite-crm'));
 ...
 /* if(!$force_check && $api_check<$time){ //check validity period in settings tab
  ...
  $msg=__( 'Successfully Connected to Netsuite','contact-form-netsuite-crm' );
     if(isset($info['_time'])){
