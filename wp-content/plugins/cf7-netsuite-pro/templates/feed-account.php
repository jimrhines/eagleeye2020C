...
.vxcf_options_row .howto{
  ...
  ?>
  <div class="vx_wrap"> 
    <h2 class="vx_img_head"><img alt="<?php _e("Netsuite Feeds", 'contact-form-netsuite-crm') ?>" title="<?php _e("Netsuite Feeds", 'contact-form-netsuite-crm') ?>" src="<?php echo $this->get_base_url()?>images/netsuite-crm-logo.png?ver=1" /> <?php 
echo sprintf(__('Edit Netsuite Feed # %d','contact-form-netsuite-crm') ,$fid); 
  if(empty($id) || $new_account_id != $id){
 ?> <a href="<?php echo $new_feed_link ?>" class="add-new-h2"><?php _e("Add New Feed",'contact-form-netsuite-crm'); ?></a> 
 <?php
}
 ?>
 <a href="<?php echo $feeds_link ?>" class="add-new-h2"><?php _e("Back to Feeds",'contact-form-netsuite-crm'); ?></a>
...
if(empty($feed_name)){
  ...
  ?>
  <div class="error" id="message" style="margin-top:20px;"><?php echo wpautop(sprintf(__("We are unable to login to Netsuite with the provided API token. Please make sure they are valid in the %sSettings Page%s", 'contact-form-netsuite-crm'), "<a href='".$this->link_to_settings()."'>", "</a>")); ?></div>
  <?php
  // return;
  } 
  ?>
  ...
  <?php wp_nonce_field("vx_nonce") ?>
    <div class="vx_div">
      <div class="vx_head">
<div class="crm_head_div"> <?php _e('1. Enter feed name',  'contact-form-netsuite-crm' ); ?></div>
...
<div class="crm_clear"></div> 
  ...
  <div class="vx_row">
  <div class="vx_col1">
  <label for="vx_name" class="left_header"><?php _e("Feed Name", 'contact-form-netsuite-crm'); ?>
  ...
  <div class="vx_div">
        <div class="vx_head">
<div class="crm_head_div"> <?php _e('2. Select Netsuite Account.', 'contact-form-netsuite-crm'); ?></div>
<div class="crm_btn_div" title="<?php _e('Expand / Collapse','contact-form-netsuite-crm') ?>"><i class="fa crm_toggle_btn vx_action_btn fa-minus"></i></div>
<div class="crm_clear"></div> 
  </div>
  ...
  <div class="vx_group">
    <div class="vx_row">
  <div class="vx_col1">
  <label for="vx_account" class="left_header"><?php _e("Netsuite Account", 'contact-form-netsuite-crm'); ?>
  <?php $this->tooltip("vx_sel_object") ?>
 </label>
  </div>
  <div class="vx_col2">
  <select id="vx_account" class="crm_sel load_object" name="account" autocomplete="off">
  <option value=""><?php _e("Select a Netsuite Account", 'contact-form-netsuite-crm'); ?></option>
  <?php
  ...
  <div class="clear"></div>
    ...
    <div class="vx_div">
        <div class="vx_head">
<div class="crm_head_div"> <?php _e('3. Select Contact Form.', 'contact-form-netsuite-crm'); ?></div>
<div class="crm_btn_div" title="<?php _e('Expand / Collapse','contact-form-netsuite-crm') ?>"><i class="fa crm_toggle_btn vx_action_btn fa-minus"></i></div>
<div class="crm_clear"></div> 
  </div>
  ...
  <div class="vx_group">
    <div class="vx_row">
  <div class="vx_col1">
  <label for="vx_form_id" class="left_header"><?php _e("Contact Form Form", 'contact-form-netsuite-crm'); ?>
  <?php $this->tooltip("vx_sel_form") ?>
 </label>
  </div>
  <div class="vx_col2">
  <select id="vx_form_id" class="crm_sel load_object" name="form_id" autocomplete="off">
  <option value=""><?php _e("Select a Form", 'contact-form-netsuite-crm'); ?></option>
  <?php
  ...
  <div class="clear"></div>
    ...
    <div id="crm_ajax_div_object" style="display:none; text-align: center; line-height: 100px;"><i> <?php _e('Loading, Please Wait...','contact-form-netsuite-crm'); ?></i></div>
...
//post validation
 ...
 if($(".vx_required").length){
   ...
   var value=""; 
    ...
    attach_error.append('<div class="entry_row vx_entry_error"><div class="vx_error"><i class="fa fa-warning"></i> <?php _e('This is a required field','contact-form-netsuite-crm') ?></div></div>');
    return false;
    }  
   })  
 }
  ...
  $.post(ajaxurl,{action:'get_objects_<?php echo $this->id ?>',vx_crm_ajax:vx_crm_ajax,object:object},function(res){
  ...
  e.preventDefault(); 
    if(!confirm('<?php _e('Are you sure to remove ?','contact-form-netsuite-crm') ?>')){
      return;
  }
  ...
  if(field_name_select.length){
    ...
    if(field_name == '' || crm_fields[field_name] == ''){
     alert('<?php _e('Please Select Field Name','contact-form-netsuite-crm') ?>');
     return;   
    }  
...
if(type){ row.find('.vxc_field_option').hide(); }
  ...
  $(document).on('click','.vxc_tips',function(e){
    ...
    jQuery('#crm_note_fields').select2({ placeholder: '<?php _e('Select Field','contact-form-netsuite-crm') ?>'});
} 
...
vx_select();
  function vx_select(){
      $('#vx_add_fields_select').select2({ placeholder: '<?php _e('Select Field','contact-form-netsuite-crm') ?>'});
      add_note_sel();
  }
