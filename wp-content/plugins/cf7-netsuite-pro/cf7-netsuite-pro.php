<?php
/**
* Plugin Name: Contact Form Netsuite Plugin
* Description: Integrates <a href="https://www.crmperks.com/product/contact-form-entries-plugin/">Contact Form Entries Plugin</a> with Netsuite allowing form submissions to be automatically sent to your Netsuite account 
* Version: 1.0.2
* Requires at least: 3.8
* Tested up to: 4.9
* Author URI: https://www.crmperks.com
* Plugin URI: https://www.crmperks.com/product/contact-form-netsuite-plugin/
* Author: CRM Perks, Inc
* Text Domain: contact-form-netsuite
...
class vxcf_netsuite {
 ...
 public  $url = "https://www.crmperks.com";
  ...
  public  $id = "vxcf_netsuite";
  public  $domain = "vxcf-netsuite";
  public  $version = "1.0.2";
  public  $update_id = "6000015";
  public  $min_cf_version = "1.0";
  public $type = "vxcf_netsuite";
  public  $fields = null;
  public  $data = null;
  ...
  public static $title='Contact Form Netsuite Plugin';  
  public static $path = ''; 
  public static $slug = "";
...
load_plugin_textdomain('contact-form-netsuite-crm', FALSE,  $this->plugin_dir_name(). '/languages/' );
  ...
  $install=new vxcf_netsuite_install();
  $install->create_roles();   
   ...
   $api_file=self::$path . "includes/plugin-api.php";
...
$this->push($entry,$form,'',false);    
  ...
  */
  ...
  if($type == "admin"){
     if($ver<4.2){
  ?>
    <div class="error vx_notice notice" data-id="<?php echo $id ?>"><p style="display: table"><span style="display: table-cell; width: 98%"><span class="dashicons dashicons-megaphone"></span> <b><?php _e('Contact Form Netsuite Plugin','contact-form-netsuite-crm') ?>. </b><?php echo wp_kses_post($message);?> </span>
<span style="display: table-cell; padding-left: 10px; vertical-align: middle;"><a href="#" class="notice-dismiss" title="<?php _e('Dismiss Notice','contact-form-netsuite-crm') ?>">dismiss</a></span> </p></div>
  <?php
     }else{
  ?>
  <div class="error vx_notice notice below-h2 is-dismissible" data-id="<?php echo $id ?>"><p><span class="dashicons dashicons-megaphone"></span> <b><?php _e('Contact Form Netsuite Plugin','contact-form-netsuite-crm') ?>. </b> <?php echo wp_kses_post($message);?> </p>
  </div>    
  <?php
     }
  }else{
...
*/
  ...
  $message = sprintf(__("%sContact Form Entries%s is required. %s it today!%s", 'contact-form-netsuite-crm'), "<a href='https://www.crmperks.com/'>", "</a>", "<a href='https://www.crmperks.com/'>", "</a>");   
  }else if($installed === 2){ // not active
  $message = sprintf(__('Contact Form Entries is installed but not active. %sActivate Contact Form Entries%s to use the Contact Form Netsuite Plugin','contact-form-netsuite-crm'), '<strong><a href="'.wp_nonce_url(admin_url('plugins.php?action=activate&plugin=contact-form-entries-pro/contact-form-entries-pro.php'), 'activate-plugin_contact-form-entries-pro/contact-form-entries-pro.php').'">', '</a></strong>');  
  } else if($installed === 3){ // not supported
  $message = sprintf(__("A higher version of %sContact Form Entries%s is required. %sPurchase it today!%s", 'contact-form-netsuite-crm'), "<a href='https://www.crmperks.com/'>", "</a>", "<a href='https://www.crmperks.com/'>", "</a>");
  }  
...
return $result;
  ...
  */
  ...
  $link="N/A"; $desc=__("Added to ",'contact-form-netsuite-crm');
  ...
  $desc.=$feed['object'];
  if($feed['status'] == 5){
   $desc=sprintf(__("%s Deleted",'contact-form-netsuite-crm'),$feed['object']);   
  }
  }else if($feed['status'] == 4){
      $desc=sprintf(__("%s Filtered",'contact-form-netsuite-crm'),$feed['object']); 
  }else{
  $desc= !empty($feed['error']) ? $feed['error'] : __("Unknown Error",'contact-form-netsuite-crm');
  }
  $title=__("Failed",'contact-form-netsuite-crm');   
  if($feed['status'] == 1){
  $title=__("Created",'contact-form-netsuite-crm');   
  }else if($feed['status'] == 2){
  $title=__("Updated",'contact-form-netsuite-crm');   
  }else if($feed['status'] == 4){
  $title=__("Filtered",'contact-form-netsuite-crm');   
  }else if($feed['status'] == 5){
  $title=__("Deleted",'contact-form-netsuite-crm');   
  }
  ...
  */
  ...
  printf('<input type="checkbox" name="'.$this->id.'_update" id="'.$this->id.'_update" value="1" /><label for="'.$this->id.'_update" title="%s">%s</label><br /><br />', __('Create or update this entry in Netsuite. The fields will be mapped according to the form feed settings.', 'contact-form-netsuite-crm'), __('Send to Netsuite', 'contact-form-netsuite-crm'));
  } else {
...
if(!current_user_can($this->id."_send_to_crm")){return; }
  ...
  //logs button
    ...
    $button.= '<a class="button button-large button-secondary alignright" style="margin-left:5px; margin-top:5px; " title="'.__('Go to Netsuite Logs','contact-form-netsuite-crm').'" href="'.$log_url.'">'.__('Netsuite Logs','contact-form-netsuite-crm').'</a>';
...
do_action('plugin_status_'.$this->type,'activate');  
  ...
  */
  ...
  $status=0;
  if(class_exists('vxcf_netsuite_wp')){
      $status=1;
  }else if( file_exists(WP_PLUGIN_DIR.'/contact-form-netsuite-crm/contact-form-netsuite-crm.php')) {
  $status=2;
  } 
  return $status;
  }
...
public function do_actions(){
  ...
  */
  ...
  $this->data=new vxcf_netsuite_data();
  return $this->data;
  }
...
if(empty($form['__vx_addons']) && ($event == '' || $event == 'update')){
...
if(!empty($value)){
 ...
 if(!empty($data['emp_check']) && !empty($data['emp'])){
     $feed['emp']=apply_filters('vx_assigned_user_id',$data['emp'],$this->id,$feed['id'],$entry);
 }
  ...
  if(!empty($company_res['id'])){
   $temp['vx_company_id']=array('value'=> $company_res['id'],'label'=>'company');   
...
$account=$this->post('account',$feed);
  ...
  if($this->post('id',$res)!=""){ 
      ...
      if($action == "Added"){
          if(empty($res['link'])){
  $msg=sprintf(__('Successfully Added to Netsuite (%s) with ID # %s .', 'contact-form-netsuite-crm'),$feed['object'],$res['id']);
          }else{
  $msg=sprintf(__('Successfully Added to Netsuite (%s) with ID # %s . View entry at %s', 'contact-form-netsuite-crm'),$feed['object'],$res['id'],$res['link']);
          }
  $screen_msg=__( 'Entry added in Netsuite', 'contact-form-netsuite-crm');
      }else{
            if(empty($res['link'])){
  $msg=sprintf(__('Successfully Updated to Netsuite (%s) with ID # %s . View entry at %s', 'contact-form-netsuite-crm'),$feed['object'],$res['id'],$res['link']);   
            }else{
  $msg=sprintf(__('Successfully Updated to Netsuite (%s) with ID # %s .', 'contact-form-netsuite-crm'),$feed['object'],$res['id']);   
            }
          if($event == 'delete'){  
     $screen_msg=__( 'Entry deleted from Netsuite', 'contact-form-netsuite-crm');
          }else{
     $screen_msg=__( 'Entry updated in Netsuite', 'contact-form-netsuite-crm');
          }
          }
   
  
  }else if($this->post('status',$res) == 4){
  $screen_msg=$msg=__( 'Entry filtered', 'contact-form-netsuite-crm');    
  }else{
  $status=0; $screen_msg_class="error";
  $screen_msg=__('Errors when adding to Netsuite. Entry not sent! Check the Entry Notes below for more details.' , 'contact-form-netsuite-crm' );
  if($log_id!=""){
      //message for  bulk actions in logs
  $screen_msg=__('Errors when adding to Netsuite. Entry not sent' , 'contact-form-netsuite-crm' );    
  }
  $msg=sprintf(__('Error while creating %s', 'contact-form-netsuite-crm'),$feed['object']);
...
//  } 
  ...
  $log_link=' <a href="'.$log_url.'" class="vx_log_link" data-id="'.$insert_id.'">'.__('View Detail','contact-form-netsuite-crm')."</a>";
 $screen_msg.=$log_link;
    }
    if($post_comment){
...
//$this->add_note($entry["id"], $msg);
  ...
  */
  ...
  $detail["Form Id"]=$form['id'];
    ...
    $email_info=array("msg"=>$info['msg'],"title"=>__('Netsuite','contact-form-netsuite-crm')." Error","info_title"=>"More Detail","info"=>$detail);
...
$objects=array('Customer'=>'Customer','Contact'=>'Contact','Task'=>'Task','SalesOrder'=>'SalesOrder','Invoice'=>'Invoice','PhoneCall'=>'Phone Call','SupportCase'=>'Support Case');
  return $objects;    
 }
...
$vxcf_netsuite=new vxcf_netsuite();
$vxcf_netsuite->instance();
$vx_cf['vxcf_netsuite']='vxcf_netsuite';
endif;
